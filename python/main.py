...
async def dataset_metadata(
        ...
        string_matching: str = Query("", description = 'Both options are case sensitive. Default is "Equals."', enum=["Contains", "Equals"]),
        key: str = Query(
    ...
    if (key == "" or value == "") and jq_query == "":
        ...
        keys = key.split(" ")
        if len(keys) > 1:                
            return jq(f'map_values(select(. != "No metadata.") | select({keys[0]} | if type == "array" then select(.[] {keys[1]} | if type == "array" then select(.[] | {string_matching}) else select(. | {string_matching}) end) else select({keys[1]} | {string_matching}) end)) | keys').transform(data)
        else:
            return jq(f'map_values(select(. != "No metadata.") | select({keys[0]} != null) | select({keys[0]} | if type == "array" then (.[] | {string_matching}) else {string_matching} end)) | keys').transform(data)
...
async def download(
    ...
    value: str ="",
    jq_query: str = "",
    resource: Union[list[str], None] = Query(default=None, description= "Options include: CSV, pipeline_dependencies, metadata"), 
    version: str = "latest"
        ):
    ...
    if dataset_ids != None:
        ...
        if (key == "" or value == "") and jq_query == "":
            ...
            keys = key.split(" ")
            if len(keys) > 1:                
                dataset_list = jq(f'map_values(select(. != "No metadata.") | select({keys[0]} | if type == "array" then select(.[] {keys[1]} | if type == "array" then select(.[] | {string_matching}) else select(. | {string_matching}) end) else select({keys[1]} | {string_matching}) end)) | keys').transform(data)
            else:
                dataset_list = jq(f'map_values(select(. != "No metadata.") | select({keys[0]} != null) | select({keys[0]} | if type == "array" then (.[] | {string_matching}) else {string_matching} end)) | keys').transform(data)

    async def main():
        tasks = []
        for dataset in dataset_list:
            if "metadata" in resource:
                task = asyncio.create_task(asyncio.coroutine(get_download)(dataset_name = dataset, version = version, resource = resource))
                tasks.append(task) 
            else:
                task = asyncio.create_task(asyncio.coroutine(get_download)(dataset_name = dataset, version = version, resource = resource))
                tasks.append(task) 
        ...
        mem_zip = BytesIO()
        zip_sub_dir = "rambo" + "_dependencies"
